...
function validateConfig(config) {
  ...
  if (missing.length > 0) {
    throw new Error(
      `Variaveis de ambiente do MySQL ausentes: ${missing.join(", ")}`,
    );
  }
}
...
class MySQLDatabase {
  ...
  async execute(sql, params = []) {
    ...
    try {
      return await this.pool.execute(sql, safeParams);
    } catch (err) {
      if (
        err.code === "ER_DUP_KEYNAME" &&
        /^\s*CREATE\s+(UNIQUE\s+)?INDEX/i.test(sql)
      ) {
...
const dbAll = (db, sql, params = []) => db.all(sql, params);
const dbGet = (db, sql, params = []) => db.get(sql, params);
const dbRun = (db, sql, params = []) => db.run(sql, params);
